...
deleteCategorie(id, deleteOk){
    ...
    var req = new Request('http://localhost:8000/api/categories/' + id, parameters);
    fetch(req)
    .then(response => {
            if(response.status === 200){
                deleteOk();
            ...
            <div>
                <Router history={history}>
                    <Switch>
                        ...
                        <Route path="/main/meubles/categories/modif" component={() =><FormulaireCategorie ajout={false} onCancel={this.redirect} onSubmit={this.modificationConfirmed}/>}/>
