...
import Facture from '../Contients/Facture';
...
export default class Commandes extends Component {
    ...
    fetch(req).then(response => {
      response.json().then(data => {
        ...
        data.forEach(commande => {
          ...
          temp["nomclient"] =
            commande.clientNumClient.nomClient +
            " " +
            commande.clientNumClient.prenomClient;
  ...
  patchEtatCommande(id, changeOk) {
    ...
    var req = new Request(
      "http://localhost:8000/api/commandes/" + id + "/livree",
      parameters
    );
    fetch(req).then(response => {
  ...
  handlePanier(event) {
    ...
    history.push("/main/commandes/contients/" + num);
  }

  handleFacture(event) {
    var num = event.currentTarget.id;
    history.push("/main/commandes/facture/" + num);
  }

  handleModification(event) {
    var num = event.currentTarget.id;
    this.setState({ modifyID: num });
    history.push("/main/commandes/modif/" + num);
  }
  ...
  goToAjout() {
    history.push("/main/commandes/ajout");
  }
  ...
  toggleModalChangeConfirmation() {
    this.setState({
      modalChangeConfirmation: !this.state.modalChangeConfirmation
    });
  }
  refresh() {
    this.getCommandes();
  ...
  constructor(props) {
    ...
    this.handleFacture = this.handleFacture.bind(this);
    ...
    this.toggleModalChangeConfirmation = this.toggleModalChangeConfirmation.bind(
      this
    );
      ...
      <div>
        <Router history={history}>
          <Switch>
            ...
            <Route
              exact
              path="/main/commandes/facture/:num"
              component={Facture}
            />
            <Route
              path="/main/commandes/ajout"
              component={() => (
                <FormulaireCommande
                  ajout
        ...
        />
        <Confirmation
          text=" Attention vous ne pouvez plus revenir en arriere veuillez verifier ? "
          onNo={this.toggleModalChangeConfirmation}
          onYes={() => {
            this.etatChangeConfirmed(this.state.modifyID);
          }}
          isOpen={this.state.modalChangeConfirmation}
          toggle={this.toggleModalChangeConfirmation}
        />
      </div>
    );
  }
