...
export default class Contients extends Component {
    ...
    fetch(req).then(response => {
      response.json().then(data => {
        ...
        data.forEach(commande => {
          ...
          temp["prixUnitaire"] = this.format.formatPrix(
            commande.meubleNumSerie.prix + ""
          );
          temp["prix"] = this.format.formatPrix(
            commande.meubleNumSerie.prix * commande.nombreCommande + ""
          );
        ...
        this.setState({
          dataCommandesMeubles: commandesMeubles,
          loading: false
        });
      });
    });
  ...
  goToAjout() {
    history.push(
      "/main/commandes/contients/" + this.props.match.params.num + "/ajout"
    );
  }
      ...
      <div>
        <Router history={history}>
          <Switch>
            ...
            <Route
              path="/main/commandes/contients/:num/modif"
              component={() => (
                <FormulaireCommandesMeubles
                  ajout={false}
                  onCancel={this.redirect}
